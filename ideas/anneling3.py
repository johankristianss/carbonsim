...
def calculate_emissions_with_wait(solution, workloads, clusters):
    total_emissions = 0
    ...
    for workload, cluster in solution.items():
        ...
        if start_time > workload_data["deadline"]:
            # Assign a penalty for violating the deadline
            return float('inf')
...
# Simulated Annealing Algorithm with waiting times
def simulated_annealing(workloads, clusters, max_iter=1000, initial_temp=100, cooling_rate=0.95):
    ...
    current_cost = calculate_emissions_with_wait(current_solution, workloads, clusters)
    ...
    for iteration in range(max_iter):
        ...
        new_cost = calculate_emissions_with_wait(new_solution, workloads, clusters)
