...
class TestProcessPool(unittest.TestCase):
    def test_pool(self):
        ...
        process = Process("test_process_2", 1, 0, 60, "../filtered_workloads/1.csv", workloads_stats_dir)
        res = pool.add_process(process)
        self.assertEqual(res, True)
        
        process = Process("test_process_3", 2, 0, 60, "../filtered_workloads/2.csv", workloads_stats_dir)
        res = pool.add_process(process)
